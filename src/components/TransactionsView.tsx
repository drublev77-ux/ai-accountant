...
export function TransactionsView() {
	...
	const sendCryptoMutation = useMutation({
		mutationFn: async (data: typeof cryptoFormData) => {
			...
			if (!ethAddressRegex.test(data.to_address)) {
				throw new Error("Invalid Ethereum address format");
			}

			// ✅ SECURITY FIX: Validate amount is positive and reasonable
			const ethAmount = parseFloat(data.amount_eth);
			if (Number.isNaN(ethAmount) || ethAmount <= 0) {
				throw new Error("Invalid amount: must be greater than 0");
			}
			if (ethAmount > parseFloat(balance || "0")) {
			...
			// ✅ SECURITY FIX: Add timeout for transaction confirmation (max 5 minutes)
			toast.info("Waiting for blockchain confirmation... (this may take 1-3 minutes)");

			const receipt = await Promise.race([
				tx.wait(),
				new Promise<null>((_, reject) =>
					setTimeout(() => reject(new Error("Transaction confirmation timeout (5 min)")), 300000)
				)
			]);
			...
			// ✅ SECURITY FIX: Verify transaction status
			if (receipt.status === 0) {
				throw new Error("Transaction reverted on blockchain");
			}

			// ✅ FIX: Get current ETH/USD price for accurate USD amount
			// In production, fetch real-time price from API (e.g., CoinGecko, Binance)
			const ETH_USD_PRICE = 2500; // Mock price - replace with real API call
			const usdAmount = ethAmount * ETH_USD_PRICE;

			// Save transaction to database
			const transactionData: Partial<TransactionModel> = {
				user_id: userId || "",
				type: TransactionType.Expense, // ✅ FIX: Crypto sends are always expenses
				category: data.category,
				amount: usdAmount, // ✅ FIX: Store USD equivalent, not ETH amount
				description: `${data.description || "Crypto Payment"} | TX: ${receipt.hash} | From: ${account} | To: ${data.to_address} | ${ethAmount} ETH @ $${ETH_USD_PRICE}/ETH = $${usdAmount.toFixed(2)} USD | Network: Ethereum | Status: Confirmed (Block ${receipt.blockNumber})`,
				date: data.date,
			};
			...
			if (isOnline) {
				...
				// ✅ FIX: Save offline if network is down
				await syncManager.saveOffline(transactionData as TransactionModel, 'create');
				return [transactionData as TransactionModel];
			}
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["transactions"] });
			toast.success("Crypto transaction completed & recorded successfully!");
			resetCryptoForm();
