...
export function TaxCalculatorView() {
	...
	const { data: taxProfile, isLoading: profileLoading } = useQuery({
		...
		queryFn: async () => {
			...
			const profile = results.filter(p => p.user_id === userId)[0] || null;

			if (profile) {
				const deductions = profile.deductions ? JSON.parse(profile.deductions) : {};
				setProfileData({
					jurisdiction: profile.jurisdiction,
					filingStatus: profile.filing_status.toString(),
					dependents: deductions.dependents?.toString() || "0",
					estimatedTaxRate: deductions.estimatedTaxRate?.toString() || "",
					yearlyIncome: deductions.yearlyIncome?.toString() || "",
				});
			}
	...
	const handleSaveProfile = () => {
		...
		const deductionsData = JSON.stringify({
			dependents: parseInt(profileData.dependents),
			estimatedTaxRate: parseFloat(profileData.estimatedTaxRate),
			yearlyIncome: parseFloat(profileData.yearlyIncome),
		});
