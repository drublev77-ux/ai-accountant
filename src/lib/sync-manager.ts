...
export class SyncManager {
	...
	private setupNetworkListeners(): void {
		...
		window.addEventListener('offline', () => {
			this.isOnline = false;
			logger.log('Network: Offline - data will be queued');
	...
	async sync(): Promise<void> {
		...
		try {
			...
			logger.log(`Processing ${queue.length} items in sync queue`);
			...
			logger.log(`Pulled ${remoteTransactions.length} transactions from server`);
